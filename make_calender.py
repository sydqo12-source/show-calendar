...
def push_to_github():
    ...
    try:
        ...
        try:
            subprocess.run(["git", "commit", "-m", "Update calendar design details"], check=True)
...
<head>
    ...
    <style>
        ...
        .header-wrapper {{
            ...
            align-items: center;
            margin-bottom: 10px; /* [수정] 연도월과 지역선택지 사이 간격 좁힘 */
        }}

        .main-title {{ 
            font-size: 38px; /* [수정] 대제목 키움 */
            font-weight: 800; 
            color: #343a40; 
            margin-bottom: 25px; /* [수정] 대제목과 연도 사이 간격 늘림 */
            word-break: keep-all;
            text-align: center;
        }}
        ...
        .nav-row {{
            display: flex;
            align-items: center;
            gap: 10px; /* [수정] 화살표와 연도 사이 간격 좁힘 */
        }}

        .sub-title {{ 
            font-size: 28px; /* [수정] 연도월 폰트 줄임 (대제목보다 작게) */
            font-weight: 800; 
            color: #495057; 
        }}
        ...
        /* [수정] PC 날짜 숫자: 흐림 제거 (선명하게) */
        .date-num {{ 
            font-weight: 800; font-size: 14px; 
            color: #212529; /* [수정] 진한 색상 유지 */
        ...
        @media screen and (max-width: 768px) {{
            ...
            .main-title {{ font-size: 28px; margin-bottom: 15px; }} /* 대제목 */
            .sub-title {{ font-size: 24px; }} /* 연도월 (더 작게) */
            .nav-row {{ gap: 8px; }} /* 화살표 간격 좁게 */
            ...
            /* 비활성 상태 (선택됐지만 공연이 안 보이는 날) -> 기본 상태 유지 */
