...
def get_content_html(row_data):
    """각 일정의 HTML 박스를 생성 (data-link2 추가)"""
    region = str(row_data.get('지역', ''))
    title = str(row_data.get('제목', ''))
    place = str(row_data.get('장소', ''))
    genre = str(row_data.get('장르', ''))
    
    poster_url = str(row_data.get('포스터', ''))
    booking_url1 = str(row_data.get('링크', ''))   # 기존 링크 (NOL)
    booking_url2 = str(row_data.get('link2', ''))  # 추가된 링크 (Yes24)
    
    if poster_url == 'nan': poster_url = ''
    if booking_url1 == 'nan': booking_url1 = ''
    if booking_url2 == 'nan': booking_url2 = ''

    raw_time = str(row_data.get('오픈일시', ''))
    ...
    <div class="event-box" 
         ...
         data-link="{booking_url1}"
         data-link2="{booking_url2}"
...
def push_to_github():
    ...
    try:
        ...
        try:
            subprocess.run(["git", "commit", "-m", "Update calendar with merged data"], check=True)
...
def main():
    ...
    try:
        # 1. 시트 데이터 로드
        df_base = pd.read_excel(FILENAME, sheet_name=0).fillna({'장르': '기타', '지역': '(기타)', '포스터': '', '링크': ''})
        try:
            df_new = pd.read_excel(FILENAME, sheet_name=1).fillna({'장르': '기타', '지역': '(기타)', '포스터': '', '링크': ''})
        except:
            print("Sheet2가 없습니다. 기존 데이터로만 생성합니다.")
            df_new = pd.DataFrame()

        print("데이터 병합 및 중복 체크 중...")

        # 2. 데이터 병합 로직 (In-Memory)
        base_data = df_base.to_dict('records')
        
        # 미리 날짜 파싱해두기 (비교 속도 및 정확도 향상)
        for item in base_data:
            item['parsed_date'] = smart_parse_date(item['오픈일시'])
            item['norm_title'] = normalize_text(item['제목'])
        ...
        if not df_new.empty:
            for idx, row in df_new.iterrows():
                ...
                # 비교할 대상 데이터 준비
                new_date_parsed = smart_parse_date(row['오픈일시'])
                new_title_norm = normalize_text(row['제목'])
                
                is_matched = False

                # '중복' 이라고 적혀있는 경우 매칭 시도
                if first_col_val == '중복':
                    for base_item in base_data:
                        # 1차 필터: 날짜가 같아야 함 (년,월,일)
                        # 날짜가 파싱되지 않으면 매칭 실패로 간주
                        if base_item['parsed_date'] != new_date_parsed or new_date_parsed == (None, None, None):
                            continue

                        # 2차 필터: 제목 유사성 (한쪽이 다른쪽을 포함하는지)
                        if (new_title_norm in base_item['norm_title']) or (base_item['norm_title'] in new_title_norm):
                            # 매칭 성공! -> link2에 Yes24 링크 저장
                            base_item['link2'] = row['링크']
                            is_matched = True
                            break
                
                # 매칭되지 않았거나, '중복' 표시가 없는 경우 -> 그냥 새 공연으로 추가
                if not is_matched:
                    new_item = row.to_dict()
                    
                    # [수정된 부분] 추가되는 데이터에도 비교용 키(parsed_date, norm_title)를 반드시 넣어야 함
                    new_item['parsed_date'] = new_date_parsed
                    new_item['norm_title'] = new_title_norm
                    
                    base_data.append(new_item)

        # 병합된 데이터를 다시 DataFrame으로 변환
        df = pd.DataFrame(base_data)

        # 3. 날짜 파싱 및 정렬
        if '오픈일시' not in df.columns:
            print("오류: 데이터에 '오픈일시' 컬럼이 없습니다.")
            return
        ...
        # HTML 템플릿의 나머지 부분 (CSS, JS 등은 위에서 정의한 변수 사용)
        # f-string 포맷팅을 위해 중괄호를 두 번({{}}) 써야 하는 부분 주의
        
        # (HTML 문자열 생성 코드는 기존 코드와 동일하므로 여기서는 생략하고 핵심 로직만 수정했습니다.
        # 기존 코드의 full_html = f"""...""" 부분은 그대로 두시면 됩니다.)
        
        # HTML 저장 부분 복원 (사용자 코드의 뒷부분)
        full_html = f"""
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <style>
        ...
        .modal-overlay {{
            ...
            padding-bottom: 50px; backdrop-filter: blur(3px); overflow-y: auto; 
        }}
        .modal-content {{
            background: white; width: 90%; max-width: 350px;
            border-radius: 12px; overflow: hidden;
            box-shadow: 0 10px 25px rgba(0,0,0,0.2);
            animation: popUp 0.2s ease-out; margin-bottom: 30px; 
        }}
        ...
        .btn-yes24 {{ background-color: #1a73e8; color: white; }} 
        .btn-calendar {{ background-color: #fa5252; color: white; flex-basis: 100%; }} 
...
<body class="initial-mode">
    ...
    <div id="eventModal" class="modal-overlay" onclick="closeModal(event)">
        <div class="modal-content" onclick="event.stopPropagation()">
            ...
            <div class="modal-body">
                ...
                <div class="modal-btn-group" id="btnGroup">
                    </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function openModal(element) {{
            ...
            // [수정] 통합 여부에 따른 버튼 생성 로직
            let htmlBtns = '';
            
            const hasLink1 = (ds.link && ds.link.trim() !== '' && ds.link !== 'nan');
            const hasLink2 = (ds.link2 && ds.link2.trim() !== '' && ds.link2 !== 'nan');

            if (hasLink2) {{
                // 중복(통합)된 경우: 왼쪽(NOL) + 오른쪽(yes24)
                if (hasLink1) htmlBtns += `<a href="${{ds.link}}" target="_blank" class="modal-btn btn-booking">NOL 티켓</a>`;
                htmlBtns += `<a href="${{ds.link2}}" target="_blank" class="modal-btn btn-yes24">yes24 티켓</a>`;
            }} else {{
                // 일반: 기존 버튼명 사용 (예매 안내 페이지)
                if (hasLink1) {{
                    htmlBtns += `<a href="${{ds.link}}" target="_blank" class="modal-btn btn-booking">예매 안내 페이지</a>`;
                }}
            }}
            
            // 일정 추가 버튼은 항상 표시
            htmlBtns += `<button onclick="openConfirmModal()" class="modal-btn btn-calendar">일정 추가</button>`;
            
            btnGroup.innerHTML = htmlBtns;
